#!/usr/bin/env python3
"""
Nek5000 Field File Converter

A script to convert Nek5000 binary field files to VTK or HDF5 formats.
Reads reference quantities from a JSON file and processes the binary data
with proper scaling.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Convert Nek5000 field files to VTK or HDF5 format",
    ...
    parser.add_argument(
        "--vtk",
        action="store_true",
        help="Write VTK output file"
    )
    parser.add_argument(
        "--vtk-output",
        default="output.vtk",
        help="VTK output file name"
    )
    parser.add_argument(
        "--vtk-format",
        default="vtk42",
        choices=["vtk", "vtk42", "vtu"],
        help="VTK file format"
    )
    parser.add_argument(
        "--hdf5",
        action="store_true",
        help="Write HDF5 output file"
    )
    parser.add_argument(
        "--hdf5-output",
        default="output.h5",
...
def _as_unicode(string: StringOrBytes) -> str:
    """Convert bytes to unicode string if necessary."""
...
def _as_tuple_of_ints(seq):
    """Convert sequence to tuple of integers."""
...
class ElemData:
    ...
    def __init__(self, ndim, nel, lr1, var, ncellv, nbc=0, dtype="float64"):
        ...
        nel : int
            Number of elements
        lr1 : tuple
            Polynomial orders (lx, ly, lz)
        var : tuple
            Number of variables for each field type
        ncellv : int
...
class Header:
    ...
    This relies on the package attrs and its ability to do type-validation
    and type-conversion of the header metadata.
    """
    ...
    def _variables_to_nb_vars(self) -> Optional[Tuple[int, ...]]:
        """Convert variable string to number of variables tuple."""
        ...
        if not nb_dims:
            logger.error("Uninitialized nb_dims")
            return None
    ...
    def _nb_vars_to_variables(self) -> Optional[str]:
        """Convert number of variables tuple to variable string."""
    ...
    def as_bytestring(self) -> bytes:
        """Convert header to bytestring format."""
...
def read_header(path_or_file_obj: Union[PathLike, BinaryIO]) -> Header:
    """
    Make a Header instance from a file buffer opened in binary mode.
    
    Parameters
    ----------
    path_or_file_obj : PathLike or BinaryIO
        Path to file or opened file object in binary mode
...
def readnek(fname, ref_values=None, dtype="float64", skip_vars=()):
    """
    Read binary data from the nek5000 binary format.
    ...
    skip_vars : tuple[str]
        Variables to skip. Valid values: ("x", "y", "z", "ux", "uy", "uz",
        "pressure", "temperature", "s01", "s02", ..., "vx", "vy", "vz", "p", "t")
    ...
    def skip_elements(nb_elements=1):
        """Skip specified number of elements in the file."""
    ...
    geometry_vars = "x", "y", "z"
    nb_vars = h.nb_vars[0]
    skip_condition = tuple(
        geometry_vars[idim] in skip_vars for idim in range(nb_vars)
    )
    ...
    skip_condition = tuple(
        scalar_vars[ivar] in skip_vars for ivar in range(nb_vars)
    )
    ...
    ix, iy, iz, iel = np.ix_(
        ix_range[:-1], iy_range[:-1], iz_range[:-1], elmap
    )
    
    if h.nb_dims == 3:
        data.cell_vert[:, 0] = (
            of + iz * ly1 * lx1 + iy * lx1 + ix
        ).ravel()
        data.cell_vert[:, 1] = (
            of + iz * ly1 * lx1 + iy * lx1 + ix + 1
        ).ravel()
        data.cell_vert[:, 2] = (
            of + iz * ly1 * lx1 + (iy + 1) * lx1 + ix + 1
        ).ravel()
        data.cell_vert[:, 3] = (
            of + iz * ly1 * lx1 + (iy + 1) * lx1 + ix
        ).ravel()
        data.cell_vert[:, 4] = (
            of + (iz + 1) * ly1 * lx1 + iy * lx1 + ix
        ).ravel()
        data.cell_vert[:, 5] = (
            of + (iz + 1) * ly1 * lx1 + iy * lx1 + ix + 1
        ).ravel()
        data.cell_vert[:, 6] = (
            of + (iz + 1) * ly1 * lx1 + (iy + 1) * lx1 + ix + 1
        ).ravel()
        data.cell_vert[:, 7] = (
            of + (iz + 1) * ly1 * lx1 + (iy + 1) * lx1 + ix
        ).ravel()
    else:
...
def read_reference_values(json_filename):
    """
    ...
    return ref_values, scalars_names


def write_vtk_file(nekdata, scalars_names, output_filename, file_format="vtk42"):
    """
    ...
    ----------
    nekdata : ElemData
        Nek5000 data structure
    scalars_names : list
        Names of scalar fields
    output_filename : str
        Output VTK filename
    file_format : str
        VTK file format (vtk, vtk42, or vtu)
    """
    print(f'\n>>>> Writing VTK file: {output_filename}', flush=True)
    t0 = time.time()
...
def write_hdf5_file(nekdata, scalars_names, output_filename):
    """
    ...
    """
    print(f'\n>>>> Writing HDF5 file: {output_filename}', flush=True)
    t0 = time.time()
...
def main():
    ...
    nekdata = readnek(
        ...
        ref_values,
        dtype=args.dtype,
        skip_vars=tuple(args.skip_vars)
    )
    
    if nekdata == -1 or nekdata == -3:
        print("Error reading Nek5000 file")
        sys.exit(1)
    
    t1 = time.time()
    print(f'Time elapsed [sec]: {t1 - t0:.1f}', flush=True)
    
    # Write VTK file if requested
    if args.vtk:
        write_vtk_file(
            nekdata,
            scalars_names,
